...
// Add click event listener to the fish name after it's in the DOM
...
c.innerHTML = `<img src="${p.image}" class="gallery-photo" /> <div>${p.caption}</div> <div>${p.loc}</div> <button onclick="deletePost(${p.timestamp})">Delete</button>`;
